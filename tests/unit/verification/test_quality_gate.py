...
def test_single_language_not_applicable_l4a_does_not_fail_gate():
    """REQUIREMENT: an unavoidable ``not_applicable`` L4a parity check (single
    language, nothing to compare) must NOT drag the mechanical score below the
    1.0 threshold, and must NOT mislabel a pending-semantic state as ``failed``.

    Regression for the P1: single-language runs emit one ``l4a_mechanical``
    check with ``status="not_applicable"`` / ``verified=False``. It was counted
    in ``mechanical_total`` but excluded from ``mechanical_passed``, so the
    score was e.g. 3/4 = 0.75, ``meets_score`` False, and the outer
    ``not meets_score -> failed`` branch fired BEFORE the ``pending_llm_layers``
    branch — turning a genuinely-passing mechanical plane with only an
    un-reviewed LLM layer into ``failed``/exit 1 instead of
    ``pending_semantic_review``. This violates the honesty contract that a
    pending semantic state never flips to ``failed``.
    """
    ...
    # L3/L5 pending LLM audit; L4 single-language (l4a + l4b not_applicable).
    layers["L3"] = LayerReport(
        layer="L3", name="Behavior",
        checks=[VerificationCheck(
            layer="L3", target="d.md", claim_type="behavior",
            claim_text="b", verified=False, status="pending", detail="audit pending",
        )],
    )
    layers["L4"] = LayerReport(
        layer="L4", name="Cross-language",
        checks=[
            VerificationCheck(
                layer="L4", target="d.md", claim_type="l4a_mechanical",
                claim_text="block parity", verified=False, status="not_applicable",
                detail="single language",
            ),
            VerificationCheck(
                layer="L4", target="d.md", claim_type="l4b_semantic",
                claim_text="prose parity", verified=False, status="not_applicable",
                detail="single language",
            ),
        ],
    )
    layers["L5"] = LayerReport(
        ...
        checks=[VerificationCheck(
            layer="L5", target="d.md", claim_type="epistemic",
            claim_text="e", verified=False, status="pending", detail="audit pending",
        )],
    )
    report = ComprehensiveVerificationReport(layers=layers)
    result = evaluate_quality_gate(report, MakeWikiConfig.default(Path(".")))
    assert result.l4a_status == "not_applicable"
    # Mechanical plane genuinely passes: not_applicable parity is not a miss.
    assert result.mechanical_score == pytest.approx(1.0)
    assert result.mechanical_passed is True
    # LLM semantic layers still pending -> PENDING_SEMANTIC_REVIEW, never failed.
    assert result.verdict == "pending_semantic_review"
    assert result.passed is False
