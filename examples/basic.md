...
1. **Phase 0 (Sizing)**: Evaluated as `Tier S`. Main Agent allocates 2 subagents (EN Writer + ZH Writer).
2. **Phase 1 (Scout)**: Discovers `sample-cli greet <name>`, `--count`, and `.env.example`.
3. **Phase 2 (ReBattle)**: Single-pass multi-perspective check verifies commands against Typer AST.
