...
| `benchmark-index` | `[corpus_dir] [--output <path>] [--format json\|human]`                                                                                                                 | Validate the benchmark corpus (schema + cross-links) and deterministically (re)generate the compact stage-1 index at `<corpus>/index/benchmark-index.json`. Metadata only — never benchmark prose. |
| `benchmark-acquire` | `<ids...> [--corpus <dir>] [--force] [--timeout <sec>]`      | Licensing-gated fetch of benchmark source pages into `sources/<provider>/` with sidecar metadata. Entries marked metadata-only are ALWAYS refused; unlocking a fetch is a registry curation decision (acquire: full + license note), never a CLI bypass. |
| `verify-reference-profile` | `<profile.yaml> [--corpus <dir>] [--render <path>] [--format json\|human]` | Mechanical ReferenceProfile validation. Exit 1 marks a blocking defect: unknown benchmark id, duplicate reference, count over `benchmark.max_examples_per_page`, or a rendered payload over `benchmark.max_reference_tokens`. `--render` writes the compact advisory Markdown. |
| `benchmark-leakage` | `<wiki_dir> --profile <path>... [--corpus <dir>] [--format json\|human]` | Mechanical candidate scan of generated docs for benchmark provider terms. Output is a location-annotated CANDIDATE list — review input, never a verdict. |
